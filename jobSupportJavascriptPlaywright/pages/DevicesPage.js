 ...
 */
    ...
    /**
     * Wait for navigation after adding device
     */
    async waitForNavigationAfterAddDevice(timeout = 10000) {
        try {
    ...
    /**
     * Fill all required device fields
     */
    async fillDeviceForm(deviceData) {
        if (deviceData.externalDeviceId) {
            await this.enterExternalDeviceId(deviceData.externalDeviceId);
        }
        if (deviceData.type) {
            await this.selectType(deviceData.type);
        }
        if (deviceData.subtype) {
            await this.selectSubtype(deviceData.subtype);
        }
        if (deviceData.status) {
            await this.selectStatus(deviceData.status);
        }
        if (deviceData.disposition) {
            await this.selectDisposition(deviceData.disposition);
        }
        if (deviceData.assignedOrganization) {
            await this.selectAssignedOrganization(deviceData.assignedOrganization);
        }
    }
    ...
    /**
     * Get the current value of external device ID field
     */
    async getExternalDeviceIdValue() {
        return await this.page.inputValue(this.selectors.externalDeviceIdInput);
    }

    /**
     * Check if external device ID field is empty
     */
    async isExternalDeviceIdEmpty() {
        const value = await this.getExternalDeviceIdValue();
        return value === '';
    }

    /**
     * Take screenshot of the devices page
     */
